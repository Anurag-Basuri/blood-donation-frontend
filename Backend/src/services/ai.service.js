...
class AiService {
	...
	async findOptimalDonorMatch(requestData) {
		try {
			const { bloodType, location, urgency, quantity, specialRequirements } = requestData;
			...
			const potentialDonors = await User.find({
				...
				.select('bloodType address.location lastDonationDate donationHistory medicalInfo')
				.lean();
			...
			const donorScores = await this.calculateDonorScores(potentialDonors, requestData);
			return {
				donors: donorScores,
				searchRadius,
			...
			throw new ApiError(error.statusCode || 500, `Donor matching failed: ${error.message}`);
		}
	}
	...
	async optimizeEmergencyResponse(emergencyData) {
		try {
			const { location, bloodType, quantity, urgencyLevel } = emergencyData;
			...
			const allSources = [...hospitals, ...ngos].map(source => ({
				...source,
				distance: this.calculateDistance(location, source.address.location.coordinates),
			...
			const strategy = await this.generateEmergencyStrategy(allSources, emergencyData);

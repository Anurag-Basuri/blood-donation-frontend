...
const createBloodRequest = asyncHandler(async (req, res) => {
	const { bloodGroups, urgencyLevel, requiredBy, patientInfo, hospitalId, requestNotes } =
		req.body;
...
const getEmergencyRequests = asyncHandler(async (req, res) => {
	...
	return res
		.status(200)
		.json(new ApiResponse(200, emergencyRequests, 'Emergency requests fetched'));
});
...
const trackRequest = asyncHandler(async (req, res) => {
	...
	return res
		.status(200)
		.json(new ApiResponse(200, { request, status }, 'Request details fetched'));
});

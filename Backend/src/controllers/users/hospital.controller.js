...
const registerHospital = asyncHandler(async (req, res) => {
	...
	if ([name, email, password, registrationNumber].some(field => !field?.trim())) {
	...
	return res
		.status(201)
		.json(new ApiResponse(201, hospital, 'Hospital registration pending verification'));
});
...
const createBloodRequest = asyncHandler(async (req, res) => {
	...
	await Hospital.findByIdAndUpdate(hospitalId, {
		$inc: {
			...
			'statistics.emergencyRequests': urgencyLevel === 'Emergency' ? 1 : 0,
		},
	...
	return res
		.status(201)
		.json(new ApiResponse(201, bloodRequest, 'Blood request created successfully'));
});
...
const updateBloodInventory = asyncHandler(async (req, res) => {
	...
	// Update inventory
	await hospital.updateBloodInventory(bloodGroup, operation === 'add' ? units : -units);
...
const manageNGOConnections = asyncHandler(async (req, res) => {
	...
	const hospital = await Hospital.findById(hospitalId);
	const connection = hospital.connectedNGOs.find(conn => conn.ngoId.toString() === ngoId);
	...
	return res
		.status(200)
		.json(new ApiResponse(200, { hospital }, 'NGO connection updated successfully'));
});
...
const findNearbyNGOs = asyncHandler(async (req, res) => {
	...
	return res
		.status(200)
		.json(new ApiResponse(200, nearbyNGOs, 'Nearby NGOs fetched successfully'));
});
...
const updateEmergencyContact = asyncHandler(async (req, res) => {
	...
	return res
		.status(200)
		.json(new ApiResponse(200, hospital, 'Emergency contact updated successfully'));
});

// UPDATE HOSPITAL PROFILE
const updateHospitalProfile = asyncHandler(async (req, res) => {
	const hospitalId = req.hospital._id;
	const { name, email, address, contactPerson, type, specialties, emergencyContact } = req.body;

...
const listEquipment = asyncHandler(async (req, res) => {
    ...
    const query = {
        ...
        "status.current": availability ? EQUIPMENT_STATUS.AVAILABLE : { $ne: EQUIPMENT_STATUS.DISPOSED }
    };
...
const updateEquipmentStatus = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, equipment, "Equipment status updated")
    );
});
...
const bookEquipment = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, equipment, "Equipment booked successfully")
    );
});
...
const getEquipmentHistory = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, history, "Equipment history fetched")
    );
});

...
router.post('/login', validateRequest('hospital.login'), loginHospital);
...
router.patch('/change-password', validateRequest('hospital.passwordChange'), changePassword);
...
router
	...
	.patch(validateRequest('hospital.inventoryUpdate'), updateBloodInventory)
	.get(validateRequest('hospital.inventoryQuery'), (req, res) => {
...
router.get('/nearby-ngos', validateRequest('hospital.nearbySearch'), findNearbyNGOs);

router
	.route('/ngo-connections/:ngoId')
	.post(validateRequest('hospital.connectionCreate'), (req, res) => {
		req.params.action = 'connect';
		return manageNGOConnections(req, res);
	})
	.patch(validateRequest('hospital.connectionUpdate'), (req, res) => {
		req.params.action = 'update';
		return manageNGOConnections(req, res);
	})
	.delete(validateRequest('hospital.connectionDelete'), (req, res) => {
		req.params.action = 'disconnect';
		return manageNGOConnections(req, res);
	});

// Analytics
router.get('/analytics', validateRequest('hospital.analytics'), getHospitalAnalytics);

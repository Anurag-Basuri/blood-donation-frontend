...
const NGODashboard = () => {
	...
	const DonorTable = () => {
		...
		const handleSave = id => {
			setDonors(donors.map(donor => (donor.id === id ? { ...editData } : donor)));
			setEditingId(null);
		};
				...
				<table className="w-full">
					<thead className="bg-gray-50">
						<tr>
							...
							<th className="px-6 py-3 text-left text-xs font-medium">Blood Type</th>
				...
				>
					<div className="p-4">
						<div className="flex items-center mb-8">
							...
							{sidebarOpen && <h1 className="text-xl font-bold ml-2">BloodFlow</h1>}
						</div>
						...
						<nav className="space-y-2">
							...
							<NavigationItem
								icon={<FiCalendar />}
								label="Camps"
								notifications={camps.filter(c => c.status === 'upcoming').length}
							/>
							<NavigationItem
								icon={<FiDroplet />}
								label="Blood Stock"
							...
							<NavigationItem
								...
								label="Requests"
								notifications={requests.filter(r => r.status === 'pending').length}
							/>
											...
											>
												<div>
													...
													<p className="text-sm">
														...
														{selectedCamp.donorsRegistered} donors
														registered
													</p>
												</div>
											</InfoWindow>

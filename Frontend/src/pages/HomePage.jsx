      ...
      <header className="relative z-10 bg-white/80 backdrop-blur-md border-b border-red-100 sticky top-0">
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-4">
            {isLoggedIn ? (
              <>
                <button onClick={handleDashboard} className="btn-primary flex items-center space-x-2">
                  <span>Dashboard</span>
              ...
              <>
                <button onClick={handleLogin} className="text-red-600 hover:text-red-700 font-medium">Login</button>
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-white/95 px-4 py-4 border-t border-red-100 space-y-2">
            ...
            {isLoggedIn ? (
              <>
                <button onClick={handleDashboard} className="btn-primary w-full">Dashboard</button>
                <button onClick={handleLogout} className="btn-outline w-full">Logout</button>
              </>
            ) : (
              <>
                <button onClick={handleLogin} className="btn-outline w-full">Login</button>
      ...
      <section className="relative z-10 min-h-screen flex items-center justify-center text-center px-4">
        <div className="space-y-8">
          ...
          <p className="text-xl text-gray-600 max-w-xl mx-auto">
            Join the lifeline connecting blood donors, hospitals, and NGOs in a compassionate ecosystem.
          </p>

          {/* CTA */}
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            {isLoggedIn ? (
              <button onClick={handleDashboard} className="btn-primary-lg">Go to Dashboard</button>
            ) : (
              <>
                <button onClick={handleRegister} className="btn-primary-lg">Join as Donor</button>
                <button onClick={handleLogin} className="btn-outline-lg">Login</button>
              </>
            )}
          </div>
        </div>
      ...
      <section className="py-20 bg-white/50">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { Icon: Users, title: 'Blood Donors', desc: 'Heroes who give the gift of life.' },
              { Icon: Building2, title: 'Hospitals', desc: 'Connect and request needed blood quickly.' },
              { Icon: Shield, title: 'NGOs', desc: 'Manage drives and community outreach.' }
        ...
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-4 gap-8">
          <div className="md:col-span-2">
            ...
            <p className="text-gray-400">
              Connecting hearts and saving lives through technology.
            </p>
          </div>
          <div>
